...
const GenericModal = ({
  ...
  const handleSubmit = (e) => {
    ...
    for (const field of fieldsConfig) {
      ...
      if (field.required && field.name === "image" && !formData.image_id) {
        toast.error(t("image_required"), { autoClose: 3000 });
        return;
      }
    }
    ...
    Object.keys(formData).forEach((key) => {
      if (key === "image_id" && formData[key]) {
        data.append("image", formData[key]);
      } else if (key.includes("img") && formData[key]) {
