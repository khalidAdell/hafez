...
const VideosPage = () => {
  ...
  const { data: videos = [], error: videosError } = useQuery({
    queryKey: ["videos", filters, locale],
  ...
  useEffect(() => {
    if (videosError) {
      const errorMessage = videosError?.message || t("error");
  ...
  const addVideoMutation = useMutation({
    mutationFn: (videoData) => addGallery(videoData, locale),
    onSuccess: () => {
      setIsAddModalOpen(false);
      queryClient.invalidateQueries(["videos", filters, locale]);
  ...
  const updateVideoMutation = useMutation({
    mutationFn: ({ id, videoData }) => updateGallery(id, videoData, locale),
    onSuccess: () => {
  ...
  const deleteVideoMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries(["videos", filters, locale]);
      toast.success(t("video_deleted_successfully"), { autoClose: 3000 });
    },
    onError: (err) => {
      const errorMessage = err.response?.data?.message || t("error");
      setError(errorMessage);
  ...
  const searchConfig = { field: "search", placeholder: "search_videos" };
          ...
          <CustomActions
            ...
            onEdit={async () => {
              if (row) {
                ...
                toast.error(t("no_video_selected"), { autoClose: 3000 });
              }
            }}
            onDelete={() => {
              if (row) {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <center>
        <DashboardTable columns={columns} data={videos} />
      </center>
      <GenericModal
      ...
      <GenericModal
        ...
        onSubmit={(data) => updateVideoMutation.mutate({ id: selectedVideo.id, videoData: data })}
      ...
      <DeleteModal
        ...
        onConfirm={() => deleteVideoMutation.mutate(selectedVideo.id)}
        userName={selectedVideo?.title || ""}
      />
    </div>
  );
};
