...
const PhotoPage = () => {
  ...
  const { data: photos = [], error: photosError } = useQuery({
    queryKey: ["photos", filters, locale],
  ...
  useEffect(() => {
    if (photosError) {
      const errorMessage = photosError?.message || t("error");
  ...
  const addPhotoMutation = useMutation({
    mutationFn: (photoData) => addGallery(photoData, locale),
    onSuccess: () => {
      setIsAddModalOpen(false);
      queryClient.invalidateQueries(["photos", filters, locale]);
  ...
  const updatePhotoMutation = useMutation({
    mutationFn: ({ id, photoData }) => updateGallery(id, photoData, locale),
    onSuccess: () => {
  ...
  const deletePhotoMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries(["photos", filters, locale]);
      toast.success(t("photo_deleted_successfully"), { autoClose: 3000 });
    },
    onError: (err) => {
      const errorMessage = err.response?.data?.message || t("error");
      setError(errorMessage);
  ...
  const searchConfig = { field: "search", placeholder: "search_photos" };
          ...
          <CustomActions
            ...
            onEdit={async () => {
              if (row) {
                ...
                toast.error(t("no_photo_selected"), { autoClose: 3000 });
              }
            }}
            onDelete={() => {
              if (row) {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <center>
        <DashboardTable columns={columns} data={photos} />
      </center>
      <GenericModal
      ...
      <GenericModal
        ...
        onSubmit={(data) => updatePhotoMutation.mutate({ id: selectedPhoto.id, photoData: data })}
      ...
      <DeleteModal
        ...
        onConfirm={() => deletePhotoMutation.mutate(selectedPhoto.id)}
        userName={selectedPhoto?.title || ""}
      />
    </div>
  );
};
